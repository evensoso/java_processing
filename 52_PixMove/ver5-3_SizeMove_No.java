...
String pPath1 = "D:\\09_music_create\\03_PICTURE\\99_Render\\000_Asset\\demo1.jpg";
String pPath2 = "D:\\09_music_create\\03_PICTURE\\99_Render\\000_Asset\\demo2.jpg";
...
void draw() {
  ...
  camera(eyeX, eyeY, eyeZ, centerX, centerY, centerZ, 0, 1, 0);
   ...
   String outP = root + "52_Pixcel\\" + "5_3\\" + fileName;
...
void Calc (ArrayList<Circle> circles, int[][] values1, int[][] values2) {
  ...
  for (int i = 0; i < img.height; i += pix) {
    for (int j = 0; j < img.width; j += pix) {
      ...
      float sizeR = 0.001 * sin(count/1000);
      
      lerpC = count/40;
      float lerpVal = lerp(values1[j][i], values2[j][i], lerpC)/3;
      
      push(); //これを入れないと点が一つしかでてこない★ 加えて回転しなくなったな★
      
      Color colAdd = new Color(j, i, lerpVal, 80-(lerpVal/1));           
...
void Paint(ArrayList<Circle> circles){
    ...
    for (Circle c : circles) { 
      push(); //これを入れないと点が一つしかでてこない★ 加えて回転しなくなったな★
      stroke(c.col.R, c.col.G, c.col.B, c.col.O); //stroke(c.col.G, 80); //*sin(count/10*(i*j)));
