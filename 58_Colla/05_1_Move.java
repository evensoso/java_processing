...
void setup() {
  ...
  String root = "D:\\09_music_create\\03_PICTURE\\99_Render\\000_Asset\\2\\";
  ...
  PImage[] listS = {cosmo, dtree, branch, branch,ginko, handup, inhand, jelly, opentree, shroom, yshroom, leaf};
...
void draw() {
  ...
  String outP = root + "58_Moon\\" + "3\\" + fileName;
    //----
  ...
  float noOfImages= (80); // *1080)/800;
  for(int i=0; i<noOfImages; i++){
    //print(i + ";");
    float x= random(width);
    float y= random(height);
  int chooser = floor(map(noise(x/500, y/10), 0, 1, 0, images.size()));
