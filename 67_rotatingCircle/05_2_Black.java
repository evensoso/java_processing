...
void draw() {
  ...
  String outP = root + "67_rotatingCircle_ver1-2\\" + fileName;
    //----
  ...
  for (int q = 0; q < 5; q += 1) {
    for (int j = 0; j < 1; j++) {
      ...
      for (int i = 0; i < num; i += 2) {
        ...
        push();
        translate(v_planet.get(i).x, v_planet.get(i).y, z_plus + count*10);
        rotateX(sin(-t) / 20);
        rotateY(cos(5*t) / 20);
        rotateZ(sin(5 * t + q));
      ...
      for (int i = 0; i < num; i += 20) {
        ...
        fill(0);
        push();
        translate(v_planet.get(i).x + x_plus, v_planet.get(i).y + y_plus, z_plus + count*10);
        rotateX(t);
        rotateY(t);
        rotateZ(t);
