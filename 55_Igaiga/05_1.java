...
float L ; float R = 3*sqrt((4*PI*(200*200)/Nmax)/(2*sqrt(3))) ;
  ...
  for ( N = 0 ; N <= Nmax ; N++ ){
     for ( NN = N+1 ; NN <= Nmax ; NN++ ){
        ...
        if ( L < R ){
          ...
          stroke(125+(Z[N]/20),125+(Z[N]/2),125+(Z[N]/2), 40) ; 
     ...
     KZ = Z[N] ; KX = X[N] ;
     Z[N] = (KZ*cos(float(300-XX)/10000))-(KX*sin(float(300-XX)/10000)) ;
     X[N] = (KZ*sin(float(300-XX)/10000))+(KX*cos(float(300-XX)/10000)) ;
     KZ = Z[N] ; KY = Y[N] ;
     Z[N] = (KZ*cos(float(300-YY)/10000))-(KY*sin(float(300-YY)/10000)) ;
     Y[N] = (KZ*sin(float(300-YY)/10000))+(KY*cos(float(300-YY)/10000)) ;
     dV[N] = dV[N] - (V[N]*HH) ; 
...
void kicked(){
  ...
  Lmin = 600 ; NN = 0 ;
  for ( N = 0 ; N <= Nmax ; N++ ){
     L = sqrt(((XX-(300+X[N]))*(XX-(300+X[N])))+((YY-(300+Y[N]))*(YY-(300+Y[N])))) ;
