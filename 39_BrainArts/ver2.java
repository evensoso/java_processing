...
String outP = "D:\\09_music_create\\03_PICTURE\\99_Render\\39_BrainArt\\2\\39_BrainArt-30_######.png";
...
ArrayList calcShape(Dna _dna) {
  ...
  for (int pCnt = 0; pCnt < pMax; pCnt++) {
    ...
    float prevX = 2 * sin(pCnt); //random(-2.0, 2.0);
    float prevY = 2 * cos(pCnt); //random(-2.0, 2.0);
...
void drawPolar(float _baseHue, ArrayList<PVector> _pvs) {
  ...
  float pRadius = sin(count + 2) * HALF_PI; //random(0.25, 1.25)
  float pOrigin = sin(count + 2); // random(-1.0, 1.0);
  ...
  for (PVector ppv : _pvs) {
    for (PVector fpv : _pvs) {
      ...
      if (distance > 0.05 && distance < 0.08) {
        ...
        stroke(
               pHue % 360.0 / 10,
               noise(10.0, fpv.x, fpv.y) * 100.0 / 10,
               noise(ppv.x, ppv.y) * 60.0 / 10,
               100.0 * random(0.1, 1)
               );
